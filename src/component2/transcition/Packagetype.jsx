...
export default function Packagetype() {
  ...
  const getData = () => {
    axios
      .get(`${baseUrl}all_package_transaction`, {
        headers: {
      ...
      .then((response) => {
        console.log(response.data.all_transactions);
  ...
  const getActiveAndinActive = () => {
    if (report === "3") {
      ...
      axios
        .get(`${baseUrl}all_package_transaction?payment_status=${report}`, {
          headers: {
            "Authorization": `Bearer ${localStorage.getItem("token")}`,
            "Content-Type": "application/json",
  ...
  const getReport = () => {
    axios
      .get(`${baseUrl}export_package_transaction/?transaction_status=${report}`, {
  ...
  const filterData = data.filter((item) => {
    console.log(item); // This will log each item being checked
    return (
      (item.booking_id &&
    ...
    <div className="content">
      <div className="row">
        <div className="col-md-12">
          <h4 className="page-title">Package Transactions</h4>
        </div>
      </div>
