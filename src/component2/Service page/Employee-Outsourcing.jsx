...
const EmployeeOutsourcing = () => {
  ...
  const handleSubmit = async (index) => {
    try {
      ...
      await axios.post(`${baseUrl}cms_elite_talent_pool`, formData, {
        headers: {
  ...
  const getdata = async () => {
    try {
      const response = await axios.get(`${baseUrl}get_cms_elite_talent_pool_admin`,{
        headers: {
          "Authorization": `Bearer ${localStorage.getItem("token")}`,
      ...
      <div className="container-fluid">
        <div className='row'>
          <div className='col-12'>
            <div className="App">
              <p className='fs-3 fw-bold text-center mt-2 pb-3'> Elite Female Talent Pool </p>
              {
