...
import { Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, TextField, MenuItem, FormControl, InputLabel, Select } from '@mui/material';
...
export default function Femalejobs() {
    ...
    const fetchData = () => {
        axios.get(`${baseUrl}get_Female_jobseeker_profile/${dataUser1}`)
            .then((response) => {
    ...
    const handleExportCandidates = (jobSeekerStatus) => {
        axios.get(`${baseUrl}export_candidate/Female?jobSeeker_status=${jobSeekerStatus}`, {
            responseType: 'blob',
        })
            .then((response) => {
    ...
    const renderJobSeekerStatus = (jobSeekerStatus) => {
        ...
        return (
            <p className={classMap[jobSeekerStatus]}>
    ...
    return (
        <div className="container main_container">
            <div className='d-flex justify-content-between mb-3'><div>
                <ArrowBack className="mb-3" style={{ cursor: "pointer" }} onClick={() => { navigate('/Admin/Job-Posting') }} /></div>
                <div>
                    ...
                    <div>
                        <Box sx={{ minWidth: 120 }}>
                            <FormControl fullWidth>
                                ...
                                >
                                    <MenuItem onClick={() => handleExportCandidates(5)}>Complete</MenuItem>
                                    <MenuItem onClick={() => handleExportCandidates(6)}>Shortlist</MenuItem>
                                    <MenuItem onClick={() => handleExportCandidates(7)}>Reject</MenuItem>
                                </Select>
                            </FormControl>
                        </Box>
                    </div>
            ...
            <TableContainer component={Paper}>
                {currentdata.length === 0 ? <p style={{ "text-align": "center", "font-size": "20px" }}>No women applied for this position.</p> : (
                    <Table>
                        <TableHead>
                        ...
                        <TableBody>
                            ...
                            {currentdata.map((item, index) => (
                                <TableRow key={index}>
                                    ...
                                    <TableCell>{item.jobSeeker_status === 1 ? (<p>Pending </p>) : (item.jobSeeker_status === 2 ? (<p>schedule interview </p>) : (item.jobSeeker_status === 3 ? (<p>assessment</p>) : (item.jobSeeker_status === 4 ? (<p className='mt-2'>HR_Discussion</p>) : (item.jobSeeker_status === 5 ? (<p>complete</p>) : (item.jobSeeker_status === 6 ? ("shortlist") : (item.jobSeeker_status === 7 ? ("reject") : ("")))))))}</TableCell>
                                    <TableCell>
