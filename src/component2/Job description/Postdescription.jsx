      ...
      const handleFileUpload = e => {
       ...
       // axios.post(`${baseUrl}addJob_Description`, {
        ...
        const handlepst = () => {
            ...
            axios.post(`${baseUrl}uploadJobFile/${data12}`, formData, {
                headers: {
                    "Authorization": `Bearer ${localStorage.getItem("token")}`
            ...
            .catch((error) => {
                toast.error(error.response?.data?.message || 'Dear HR Coordinator, you are not authorized to access this section!');
                // alert(error.response?.data?.message || 'Dear HR Coordinator, you are not authorized to access this section!');
            });
        };
    ...
    return (
        <>
            <div className='mt-4'>
                ...
                <p>Upload your Job Descriptions File</p>
            ...
            {/* <CKEditor
                ...
                /> */}
            <button className='px-3 py-2 rounded border-0 fw-bold mt-4' style={{ backgroundColor: "#dab95c", color: "white" }} onClick={handlepst}>Add Description</button>
        </div >
            <div>
            </div>
        </>
